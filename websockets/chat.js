    ...
    //   });
      ...
      socket.on("chatList", async (response) => {
          ...
          let userlist = await Promise.all(list.chatFriends.map(async (list) => {
              console.log(list)
              return {
                  user: await UserModel.findOne({ _id: list.userId }).select("name phone email onlineStatus status profile_image"),
                  chat: list.chat
              }
          }));
      ...
      socket.on('message', async (response) => {
        ...
        await new ChatModel(response).save().then(async(message) => {
          
            socket.emit('message', message);
            socket.to(response.receiverId).emit('message', message);
            
            let senderName = await UserModel.findOne({_id: response.senderId}).select('name').exec();
            let reciever = await UserModel.find({_id:response.receiverId}).select("name profile_image device_token _id").exec();
            Helper.sendNotification(reciever[0].device_token,response.senderId,response.receiverId,"Pawnder",  `New message from ${senderName.name}`,{ activity:'chats' },"chat",message.createdDate)
